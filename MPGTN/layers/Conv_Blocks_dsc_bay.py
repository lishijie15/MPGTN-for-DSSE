...
class MobileNetV1_LABAY(nn.Module):

    def __init__(self, in_channels, out_channels, scale=1.0):
        super(MobileNetV1_LABAY, self).__init__()

        input_channel = make_divisible(in_channels * scale, 8)
        last_channel = make_divisible(out_channels * scale, 8)
        self.conv1 = ConvBNReLU(input_channel, input_channel // 2, 1, 1, 0, bias=False)
        self.conv2 = ConvBNReLU(input_channel // 2, last_channel, 1, 1, 0, bias=False)
